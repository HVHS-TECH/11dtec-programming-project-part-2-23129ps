...
function getUserInfo() {
    ...
    lateFees = lateFeeRates[carIndex];  // Get the late fees from the lateFeeRates array
    if (carIndex !== -1) {
        pricePerDay = prices[carIndex]; // Get the price from the prices array
        insuranceCost = insuranceRates[carIndex];
    }
    totalCost = (pricePerDay * rentalDays) + insuranceCost; // TotalCost is generated by multiplying rentalDays with pricePerDay of the selected car and adding insuranceCost

    // Alter the totalCost if discount code is correct
